        ...
        if let collections = collections {
            collections.forEach { libraryMediaItemCollection in
                ...
                let newLibraryMedia = Media(mediaResponse:
                                                MediaResponse(id: libraryMedia.playbackStoreID,
                                                              wrapperType: wrapperType.rawValue,
                                                              kind: mediaKind.value,
                                                              name: libraryMedia.title,
                                                              artistName: libraryMedia.albumArtist,
                                                              collectionName: libraryMedia.albumTitle,
                                                              trackName: libraryMedia.title,
                                                              collectionCensoredName: libraryMedia.albumTitle,
                                                              trackExplicitness: libraryMedia.isExplicitItem ? "explicit" : "notExplicit",
                                                              trackCount: libraryMedia.albumTrackCount,
                                                              trackNumber: libraryMedia.albumTrackNumber,
                                                              trackTimeMillis: libraryMedia.playbackDuration,
                                                              primaryGenreName: libraryMedia.genre,
                                                              releaseDate: libraryMedia.releaseDate?.ISO8601Format().description,
                                                              artwork: libraryMedia.artwork?.image(at: CGSize(width: 1024, height: 1024)),
                                                              composer: libraryMedia.composer,
                                                              isCompilation: libraryMedia.isCompilation,
                                                              dateAdded: libraryMedia.dateAdded)
                )
