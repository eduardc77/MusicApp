...
private extension PlayerView {
    ...
    var smallPlayerViewDetails: some View {
        HStack {
            ...
            .foregroundStyle(model.nowPlayingItem != nil ? .primary : Color.secondary)
        }
    }
    ...
    var expandedMediaDetails: some View {
        VStack(alignment: .leading, spacing: .zero) {
            InfiniteScrollText(text: model.nowPlayingItem != nil ? model.nowPlayingItem?.trackName ?? "" : "Not Playing", explicitness: model.nowPlayingItem?.trackExplicitness ?? .notExplicit, font: UIFont.systemFont(ofSize: 20, weight: .semibold))
